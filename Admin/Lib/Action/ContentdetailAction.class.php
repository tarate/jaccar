...
　*/
class ContentdetailAction extends GlobalAction 
{
...
public function add4()
	...
	public function adds4()
	{
		...
		if($Article->create()) { 
			...
			$Article->link=$_REQUEST['link'];
			if($Article->add()){ 
            	$this->assign('jumpUrl',__APP__."/Article/index4/cid/".$_REQUEST['cid']);
				$this->success(L('_INSERT_SUCCESS_'));
            }else{ 
                $this->error(L('_INSERT_FAIL_')); 
            } 
...
public function adds6()
	{
		...
		if($Article->create()) { 
			...
			$Article->link=$_REQUEST['link'];
			if($Article->add()){ 
            	$this->assign('jumpUrl',__APP__."/Article/index6/pid/".$_REQUEST['pid']);
				$this->success(L('_INSERT_SUCCESS_'));
            }else{ 
                $this->error(L('_INSERT_FAIL_')); 
            } 
