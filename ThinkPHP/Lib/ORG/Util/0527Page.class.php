...
{//类定义开始
    ...
    public function show($isArray=false){
        ...
        if ($upRow>0){
            $upPage="<li><a href='".$url."&".C('VAR_PAGE')."=$upRow'><img src=\"/jaccar/App/Tpl/default/Public/images/page_prev.jpg\" /></a></li>";
        }else{
            $upPage="";
        }

        if ($downRow <= $this->totalPages){
            $downPage="<li><a href='".$url."&".C('VAR_PAGE')."=$downRow'><img src=\"/jaccar/App/Tpl/default/Public/images/page_next.jpg\" /></a></li>";
        }else{
            $downPage="";
        }
        // << < > >>
        ...
        for($i=1;$i<=$this->rollPage;$i++){
            ...
            if($page!=$this->nowPage){
                ...
                if($this->totalPages != 1){
                    $linkPage .= "<li style=\"color:red;font-weight:bold;\">".$page."</li>";
                }
            }
        }
        $pageStr = $upPage.$theFirst.' '.$prePage.' '.$linkPage.' '.$nextPage.' '.$theEnd.' '.$downPage.'<li style=width:100px>'.$this->totalRows.' '.$this->config['header'].'/'.$this->totalPages.'pages </li>';
        if($isArray) {
