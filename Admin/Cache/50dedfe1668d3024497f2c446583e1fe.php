...
<div class="bodyTitle">
  ...
  <div class="bodyTitleText">news list</div>
</div>
<br><h3 class="marginbot"><form method="GET" name="form1" action="/admin.php/Article/add4/parentid/<?php echo $_REQUEST['parentid']; ?>">
  <input type="hidden" name="catelist" id="catelist" value="<?php echo ($cid); ?>" />
  <input name="submit" type="submit" class="inputButton"  value="Add News"/><?php if(C("TOKEN_ON")):?><input type="hidden" name="<?php echo C("TOKEN_NAME");?>" value="<?php echo Session::get(C("TOKEN_NAME")); ?>"/><?php endif;?></form></h3>
          ...
          <table class="datalist fixwidth" > <form name="form1" method="post" action="/admin.php/Article/submit">
            <tr>
              ...
              <td><?php echo (is_array($vo)?$vo["istop"]:$vo->istop); ?></td>
              <td><?php echo (date("Y-m-d",is_array($vo)?$vo["postdate"]:$vo->postdate)); ?></td>
              <td> 
              <a href="/admin.php/Article/edit4/id/<?php echo (is_array($vo)?$vo["id"]:$vo->id); ?>/parentid/<?php echo $_REQUEST['parentid']; ?>" ><img src="/Admin/Tpl/default/Public/images/edit.gif" alt="Update" align="absmiddle" /></a>　
