...
<div class="bodyTitle">
  ...
  <div class="bodyTitleText">List</div>
</div>
<br><h3 class="marginbot"><form method="GET" name="form1" action="/admin.php/Article/add7/catename/<?php echo $_REQUEST['catename']; ?>">
  <input type="hidden" name="catelist" id="catelist" value="<?php echo ($cid); ?>" />
  <input name="submit" type="submit" class="inputButton"  value="Add Points"/><?php if(C("TOKEN_ON")):?><input type="hidden" name="<?php echo C("TOKEN_NAME");?>" value="<?php echo Session::get(C("TOKEN_NAME")); ?>"/><?php endif;?></form></h3>
          ...
          <table class="datalist fixwidth" > <form name="form1" method="post" action="/admin.php/Article/submit">
            <tr>
              ...
              <td><?php echo (is_array($vo)?$vo["username"]:$vo->username); ?></td>
              <td><?php echo (date("Y-m-d",is_array($vo)?$vo["postdate"]:$vo->postdate)); ?></td>
              <td> 
              <a href="/admin.php/Article/edit7/id/<?php echo (is_array($vo)?$vo["id"]:$vo->id); ?>/catename/<?php echo $_REQUEST['catename']; ?>" ><img src="/Admin/Tpl/default/Public/images/edit.gif" alt="Update" align="absmiddle" /></a>　
